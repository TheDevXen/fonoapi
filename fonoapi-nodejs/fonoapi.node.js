...
function getDevices(queryString, callback) {
	getDevicesWithBrand(queryString, '', callback);
}

function getDevicesWithBrand(queryString, brand, callback) {
	...
	httpreq.post(url, options, function (err, res) {
		if (err) {
			...
			callback(brand + ':' + queryString, data);
		}
	});
}
...
module.exports = {
	...
	getDevicesWithBrand: getDevicesWithBrand,
	getLatest: getLatest,
